...
locals {
  ...
  zone_id = var.enabled ? (
    var.zone_id != "" ? var.zone_id : (
      var.private_enabled && length(aws_route53_zone.private) > 0 ? aws_route53_zone.private[0].zone_id :
      length(aws_route53_zone.public) > 0 ? aws_route53_zone.public[0].zone_id : ""
    )
  ) : ""
}
